...
/** Preco medio por sessao, em centavos. */
const PRECO_SESSAO = {
...
async function main() {
  ...
  for (const a of agendaPlano) {
    ...
    const tomados = ocupadoPorDia.get(a.offset) ?? []
    const livre = (inicio) =>
  ...
  const proc = (titulo) => procedimentos.find((p) => p.title === titulo)
  ...
  /* A paciente demo tambem precisa de jornada.

     `paciente@exemplo.com` e o login que se usa para abrir o portal, e vem
     do seed de usuarios, nao da lista PACIENTES daqui. Sem isto a Jornada
     abre vazia justamente para quem entra pelo caminho obvio. */
  const pacienteDemo = await prisma.patient.findFirst({
    where: { tenantId: tenant.id, email: 'paciente@exemplo.com' },
  })
  if (pacienteDemo) {
    PLANOS.push(
      {
  ...
  for (const dados of PLANOS) {
    /* Plano da lista usa indice; o da paciente demo ja traz o id. */
    const paciente = dados.pacienteId
      ? { id: dados.pacienteId }
      : pacientes[dados.paciente]
    ...
    const campos = {
      ...
      completedAt: dados.status === 'COMPLETED' ? dia(-ultimaSessao, 11) : null,
      patientId: paciente.id,
      procedureId: procedimento.id,
      createdById: doctor.id,
      tenantId: tenant.id,
  ...
  console.log(
    `✨ Planos de tratamento: ${planosCriados}` +
      ` (${PLANOS.filter((p) => p.status === 'ACTIVE').length} em andamento,` +
      ` ${PLANOS.filter((p) => p.status === 'PAUSED').length} pausados,` +
      ` ${PLANOS.filter((p) => p.status === 'COMPLETED').length} concluídos,` +
      ` ${PLANOS.filter((p) => p.status === 'CANCELLED').length} cancelados)` +
...
Este é um texto de demonstração para exercitar a listagem e a página de publicação do site.`,
  ...
  for (const m of MODELOS) {
    ...
    const dados = { ...m, tenantId: tenant.id, createdById: doctor.id, isActive: true }
